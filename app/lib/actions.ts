...
const FormSchema = z.object({
  ...
  amount: z.coerce.number().gt(0, {
    message: 'Please enter a valid amount greater than N0.'
  }),
...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  // If everything is successful, return a success message and revalidate the path.
...
export async function signup(prevState: UserState, formData: FormData) {
  ...
  try {
    ...
    // Check for unique constraint violation (optional, generic handling for now)
    return {
